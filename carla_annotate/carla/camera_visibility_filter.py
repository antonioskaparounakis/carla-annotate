...
class CameraVisibilityFilter:
    ...
    def __init__(self, camera: carla.Sensor, world: carla.World):
        self._camera = camera
        self._world = world
        self._camera_fov_half_rad = math.radians(float(self._camera.attributes["fov"]) / 2.0)

    def filter_visible(self, bboxes: List[Tuple[carla.Actor, carla.BoundingBox]]) -> List[
        Tuple[carla.Actor, carla.BoundingBox]]:
        ...
        for actor, bbox in bboxes:
            ...
            if (
                    self._is_in_fov(camera_loc, camera_fwd, bbox_loc)
                    and self._is_in_sight(camera_loc, bbox_loc, actor)
            ):
                results.append((actor, bbox))
        return results

    def _is_in_fov(self, camera_loc: carla.Location, camera_fwd: carla.Vector3D, bbox_loc: carla.Location) -> bool:
        camera_to_bbox = bbox_loc - camera_loc
        angle = camera_fwd.get_vector_angle(camera_to_bbox)
        return angle < self._camera_fov_half_rad

    def _is_in_sight(self, camera_loc: carla.Location, bbox_loc: carla.Location, actor: carla.Actor) -> bool:
        ...
        if hit.label in actor.semantic_tags and math.isclose(dist_to_hit, dist_to_bbox, abs_tol=tolerance):
            return True
        return False
